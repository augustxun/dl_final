...
def test(loader):
    ...
    total_y = torch.LongTensor([]).to(device)
    total_pred_source = torch.FloatTensor([]).to(device)  # Predictions (logits)

    for data in loader:
        data = data.to(device)
        pred_source = model(data)  # Get logits
        pred = pred_source.max(dim=1)[1]  # Get predicted class (index)

        # Concatenate predictions and true labels
        total_pred = torch.cat((total_pred, pred), dim=0)
        total_y = torch.cat((total_y, data.y), dim=0)
        total_pred_source = torch.cat((total_pred_source, pred_source), dim=0)

        correct += pred.eq(data.y).sum().item()

    # Convert tensors to numpy for metrics calculation
    total_y_np = total_y.cpu().numpy()
    total_pred_np = total_pred.cpu().numpy()
    total_pred_source_np = total_pred_source.cpu().detach().numpy()

    # Compute metrics
    f1_res = f1_score(total_y_np, total_pred_np, average='macro')
    roc_auc_macro = roc_auc_score(
        label_binarize(total_y_np, classes=list(range(dataset.num_classes))),
        total_pred_source_np,
        multi_class='ovr',
        average='macro'
    )
